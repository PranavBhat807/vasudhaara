...
export default function ProductCard({ title, price, description, image, images, rating, id }) {
  ...
  // Handle touch events for image navigation
  ...
  return (
    <motion.div
      ...
      ref={cardRef}
      className="touch-manipulation" // Improve touch responsiveness
    >
      <Card 
        shadow="sm" 
        className="w-full h-full bg-neutral-900 border border-white/5 hover:border-pink-500/50 transition-colors"
        isPressable={false} // Disable default pressable behavior on mobile
      >
        <CardBody 
          className="overflow-visible p-0 relative group"
            ...
            {hasMultipleImages && (
              <>
                ...
                <button
                  ...
                  className={`absolute left-2 top-1/2 -translate-y-1/2 z-20 w-8 h-8 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-md text-white ${
                    isTouchDevice ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                  } transition-opacity hover:bg-black/70 active:scale-95 touch-manipulation`}
                ...
                <button
                  ...
                  className={`absolute right-2 top-1/2 -translate-y-1/2 z-20 w-8 h-8 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-md text-white ${
                    isTouchDevice ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                  } transition-opacity hover:bg-black/70 active:scale-95 touch-manipulation`}
